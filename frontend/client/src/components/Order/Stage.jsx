    ...
    <section>
      <Element
        ...
        className={`flex items-center gap-x-4 shadow p-1.5 cursor-pointer  ${
          current_stage == stage
            ? "border border-l-[7px] border-l-rose-500"
            : ""
        } border-gray-300`}
        onClick={handle_click_stage}
      >
        <span className="flex justify-center items-center bg-rose-200 rounded-full w-8 h-8">
          {stage == 1 ? (
            <Fa1
              className={`${
        ...
        <div>
          <h4
            className={`font-bold ${
              current_stage == stage ? "text-rose-700" : "text-gray-500"
            }`}
          >
            {title}
          </h4>
